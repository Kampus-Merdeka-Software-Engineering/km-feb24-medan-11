...
function displayTotalSalesPrice(arrTotalSalesPrice){
    ...
    document.getElementById("total-sales-price").textContent = '$' + formattedTotalSalesPrice;
}
...
function displayTotalUnitSales(arrTotalUnitSales){
    ...
    document.getElementById("total-unit-sales").textContent = formattedTotalUnitSales;
}
...
function displaySalesGrowthChart(arrSalesGrowth){
    ...
    arrSalesGrowth.forEach((item) => {
        ...
        boroughSalesData[item.borough][dateIndex] = (boroughSalesData[item.borough][dateIndex] || 0) + parseFloat(item.sale_price);
    });
...
fetch("JSON-file/scorecard.json").then((response) => {
...
fetch("JSON-file/chart-unite-sale-borough.json").then((response) => {
    return response.json();
}).then((data) => {
